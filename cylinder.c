...
void cylinder_base(double rgb[3], double u, double v) {
    double h = 0.05;
    if (v <= h && v >= -h) {
        rgb[0] = 0;
        rgb[1] = 0;
...
int main() {
    ...
    double vlo = -1; double vhi = 1; double vstep = 0.001;
    //r given by u(t,u,v);
    ...
    for (u = ulo; u <= uhi; u += ustep) { //step through theta
        for (v = vlo; v <= vhi; v += vstep) { //step through phi
            ...
            //normal:
            //test
            cylinder_par(a,u+du,v); D3d_mat_mult_pt(a,m,a);
            cylinder_par(b,u,v+dv); D3d_mat_mult_pt(b,m,b);
            subtract_vec(a,a,p);
            subtract_vec(b,b,p);
            x_prod(n,a,b);
            normalize(n);
            //light and color:
            fall(RGB,u/(2*M_PI),v/8);
            cylinder_base(RGB,u,v);
            //color_re(RGB,u,v);
    ...
    sprintf(fname,"images/cylinder.xwd");
