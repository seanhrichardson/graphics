...
int main() {
    ...
    double normal[3];
    //find normal:
    //sphere_par(a,uB+du,vB); D3d_mat_mult_pt(a,m,a);
    //sphere_par(b,uB,vB+dv); D3d_mat_mult_pt(b,m,b);
    //subtract_vec(a,a,p);
    //subtract_vec(b,b,p);
    //x_prod(normal,a,b);
    //normalize(normal);
    ...
    sprintf(fname,"images/sphere_with_plane.xwd");
    G_save_image_to_file(fname);
